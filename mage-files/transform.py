...
def transform(df, *args, **kwargs):
    ...
    pickup_location_dim = pickup_location_dim[['pickup_location_id', 'pickup_latitude',
                                               'pickup_longitude']]
    ...
    # i
    fact_table = df[[
        'trip_id', 'VendorID', 'store_and_fwd_flag',
        'fare_amount', 'extra', 'mta_tax', 'tip_amount',
        'tolls_amount', 'improvement_surcharge', 'total_amount'
    ]].copy()

    fact_table['datetime_id']         = df['trip_id']
    fact_table['passenger_count_id']  = df['trip_id']
    fact_table['trip_distance_id']    = df['trip_id']
    fact_table['rate_code_id']        = df['trip_id']
    fact_table['pickup_location_id']  = df['trip_id']
    fact_table['dropoff_location_id'] = df['trip_id']
    fact_table['payment_type_id']     = df['trip_id']
...
def test_output(output, *args) -> None:
    ...
    assert 'fact_table' in output, 'fact_table missing from output'
